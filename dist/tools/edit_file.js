...
export const editFileTool = tool({
    description: '파일의 특정 문자열을 다른 문자열로 교체합니다. old_string은 파일 내에서 정확히 한 번만 등장해야 합니다 (replace_all=true이면 모두 교체). 들여쓰기와 공백까지 정확히 일치해야 합니다.',
    ...
    execute: async ({ path: filePath, old_string, new_string, replace_all }) => {
        ...
        try {
            ...
            if (decision.kind === 'ask') {
                ...
                const answer = await requestApproval('edit_file', `편집: ${filePath} (${occurrences}곳 교체)`, preview);
                if (answer === 'no') {
...
function makeDiffPreview(oldS, newS) {
    const a = oldS.split('\n').slice(0, 5).join('\n');
    const b = newS.split('\n').slice(0, 5).join('\n');
    return `- ${a}\n+ ${b}`;
}
