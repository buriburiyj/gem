...
export const bashTool = tool({
    description: '셸 명령을 실행합니다 (bash -lc). 작업은 현재 프로젝트 디렉토리에서 수행됩니다. 30초 후 타임아웃됩니다. 대화형 명령이나 무한 실행 명령은 피하세요.',
    ...
    execute: async ({ command, description }) => {
        ...
        if (decision.kind === 'ask') {
            const answer = await requestApproval('bash', `${command}`, description);
            if (answer === 'no') {
