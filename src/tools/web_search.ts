...
export const webSearchTool = tool({
  description:
    '웹을 검색해 최신 정보를 가져옵니다. 뉴스, 문서, 유튜브, 최근 사건 등 학습 데이터에 없거나 최신 정보가 필요할 때 사용하세요. 결과로 제목, URL, 요약을 반환합니다.',
  ...
  execute: async ({ query, max_results }) => {
    ...
    if (!apiKey) {
      const msg =
        'TAVILY_API_KEY가 설정되지 않았습니다. https://tavily.com 에서 무료 키를 발급받아 .env에 TAVILY_API_KEY=... 로 추가하세요.';
