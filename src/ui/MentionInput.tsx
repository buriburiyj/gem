...
function findActiveMention(text: string, cursor: number) {
  ...
  while (i >= 0) {
    const ch = text[i];
    if (ch === '@') {
      if (i === 0 || /\s/.test(text[i - 1])) return { start: i, query: text.slice(i + 1, cursor) };
      return null;
    }
...
function findActiveSlash(text: string, cursor: number) {
...
async function getProjectFiles() {
  ...
  if (scanning) {
    while (scanning) await new Promise((r) => setTimeout(r, 50));
    return fileCache ?? [];
  }
  scanning = true;
  try {
...
function filterFiles(files: string[], query: string, limit = 8): string[] {
  ...
  for (const f of files) {
    ...
    if (lower.startsWith(q) || base.startsWith(q)) starts.push(f);
...
export function MentionInput({
  ...
  const fileCandidates = useMemo(
    () => (mention ? filterFiles(files, mention.query) : []),
    [mention, files],
  );
  const cmdCandidates = useMemo(
    () => (slash ? filterCommands(commands, slash.query) : []),
    [slash, commands],
  );

  const activeMode: 'file' | 'cmd' | null =
    mention && fileCandidates.length ? 'file' : slash && cmdCandidates.length ? 'cmd' : null;

  useEffect(() => {
    setHighlight(0);
  }, [mention?.query, slash?.query]);

  const acceptFile = (cand: string) => {
    if (!mention) return;
  ...
  const acceptCommand = (cand: Command) => {
    const after = value.slice(cursor);
    const inserted = '/' + cand.name;
    const newValue =
      inserted + (after.startsWith(' ') || after.length === 0 ? after : ' ' + after);
    onChange(newValue);
    ...
    <Box flexDirection="column">
      ...
      {activeMode === 'file' && (
        <Box marginLeft={2} flexDirection="column" borderStyle="round" borderColor="gray" paddingX={1}>
          <Text dimColor>@{mention!.query} — Tab/Enter 선택 · ↑↓ 이동 · Esc 취소</Text>
          {fileCandidates.map((c, i) => (
            <Box key={c}>
              <Text color={i === highlight ? '#D77757' : undefined} bold={i === highlight}>
                {i === highlight ? '▸ ' : '  '}
              </Text>
              <Text color={i === highlight ? '#D77757' : undefined}>{c}</Text>
            </Box>
          ))}
        </Box>
      )}
      {activeMode === 'cmd' && (
        <Box marginLeft={2} flexDirection="column" borderStyle="round" borderColor="gray" paddingX={1}>
          <Text dimColor>/{slash!.query} — Tab/Enter 선택 · ↑↓ 이동 · Esc 취소</Text>
          {cmdCandidates.map((c, i) => (
            <Box key={c.name}>
              <Text color={i === highlight ? '#D77757' : undefined} bold={i === highlight}>
                {i === highlight ? '▸ ' : '  '}
              </Text>
              <Text color={i === highlight ? '#D77757' : undefined}>/{c.name}</Text>
